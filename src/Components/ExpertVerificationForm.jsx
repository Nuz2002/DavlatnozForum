...
const ExpertVerificationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Because submitApplication returns the raw data object (not .data),
      // we can call it 'data' for clarity.
      const data = await submitApplication({
        ...formData,
        dateOfBirth: formData.dateOfBirth,
      });
  
      // For example, data.status = "PENDING"
  ...
  return (
    <div className="bg-blue-50 w-full flex flex-col md:flex-row gap-5 px-3 md:px-8 lg:px-16 xl:px-28 text-blue-900 min-h-screen">
      {/* Mobile Menu Button */}
      <button
        onClick={() => {} /* handle your mobile menu toggle */}
